...
export class ChatStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // Create a matchId GSI for the table
    const matchIdIndexName = 'matchId';
    table.addGlobalSecondaryIndex({
      indexName: matchIdIndexName,
      partitionKey: { name: 'matchId', type: dynamodb.AttributeType.STRING },
    });

    // generate exports names for GSI
    new cdk.CfnOutput(this, model + 'matchIdIndexArn', {
      value: table.tableArn + '/index/' + matchIdIndexName,
      exportName: model + 'matchIdIndexArn'
    });
